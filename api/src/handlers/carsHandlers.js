const { getApiCars, getCarByBrand, getCarByModel, getCarDetail, deleteCarById, createCar, editCar } = require("../controllers/carsControllers")
...
const postCar = async (req, res) => {
    ...
    try {
        const response = createCar(brand, model, year, price, img )
        res.status(200).send(response)
    } catch (error) {
...
const putCar = (req, res) => {
	...
	try {
        if(id){
            let carDetail = getCarById(id)
		    const response = editCar(brand, price, model, img, year)
            res.status(200).send(response)
        }
	} catch (error) {
		res.status(500).json({ error: error.message });
...
const deleteCar = (req, res) => {
    const { id } = req.params;
    try {
        const response = deleteCarById(id);
